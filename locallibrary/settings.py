...
INSTALLED_APPS = [
    ...
    'catalog.apps.CatalogConfig', # register new apps here. 
]
...
TEMPLATES = [
    {
        ...
        'DIRS': ['./templates', ], # add './templates' to DIRS to make the path visiable for user authentication purpose
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
...
# configure the time-zone here.
# the list of values can be found: https://en.wikipedia.org/wiki/List_of_tz_database_time_zones
...
# Alternative, a global setting can be configured, SESSION_SAVE_EVERY_REQUEST = True, to force session update after every request. 
